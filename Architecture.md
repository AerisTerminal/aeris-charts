...
Indicator output metadata is additive and binding-complete: the first output's monotonic series identity is the stable binding identity; every output reports the full structured parameters, source and optional VWAP volume source, stable output name, index, and count. The host groups and renders legends from this metadata; indicator values still come through the ordinary chart value snapshot.
...
The engine-owned crosshair overlay paints the same configurable hover marker for every visible line, area, baseline, and line-shaped indicator output at the snapped logical index. Marker coordinates, per-series colors, borders, pane ownership, and scale conversion are resolved before the shared frame reaches any backend. Bar-slot highlights and tooltip guides resolve their default tint from the current chart surface, using a light lift on dark surfaces and a dark tint on light surfaces; overlay price-scale text follows the current layout foreground. Explicit host colors remain authoritative, while implicit colors retokenize with chart options.

The engine retains semantic pane layers and their ordered primitive/point ranges, then assembles the same canonical `ChartFrame` contract from clean and rebuilt layers. The retained boundaries are underlay/grid, individual series, pane chrome, transient trading risk/reward preview regions, drawings, trading lines/segmented controls, and overlay. Preview regions therefore remain behind drawings while financially actionable lines, exact control hit zones, solid primary axis tags, and bordered protection tags remain above them and below crosshair transients. The primary series' live-price cluster stays filled; secondary series and otherwise-solid trading tags that meet its raw axis region use a chart-surface fill with their semantic color as an inside border and text. The shared axis overlap layout spaces those tags outside the primary region and emits trading tags before the primary cluster so the live price remains visually authoritative even when space is constrained. Confirmed orders never manufacture persistent risk/reward fills. Position, working-order, and confirmed-protection controls are composable segment lists resolved by one cluster layout: every cluster uses the axis-label height, shares one pane-right anchor, and expands leftward without moving its close/cancel segment. Confirmed TP and SL clusters retain quantity, projected P&L, and cancel details; hover emphasis, endpoint nodes, and action tooltips reuse the same segment geometry. Trading and axis labels share the canonical pane price transforms; WebGPU, Canvas2D, GPUI, screenshots, and native/headless consumers receive no separate trading geometry. Retention never gives a backend permission to change ordering or semantics. Host/plugin primitive callbacks use the canonical frame but conservatively rebuild the affected pane stream because their output is not engine-owned. Incremental frames are tested against forced clean rebuilds across data, interaction, scale, drawing, theme, and resize mutations.

Trading interaction is one engine-owned state machine: idle, hovering, creating protection, dragging an authoritative order through a local preview, awaiting manual confirmation, or pending host acknowledgement. These states are mutually exclusive. Pointer movement changes only the local preview; release in one-click mode or Confirm in manual mode emits one semantic intent and enters pending acknowledgement. Reject or Escape discards local interaction state, while acceptance remains visibly pending until the host pushes the resulting authoritative order/position update, which reconciles the preview without letting pointer input mutate broker-owned data. A separate active/inactive trading-group visual state owns confirmed bracket connector chrome. Successful protection acknowledgement activates the related bracket, position, or parent-order group; an empty-canvas press deactivates only that visual state, leaving broker relationships and confirmed entry, TP, and SL objects intact.
