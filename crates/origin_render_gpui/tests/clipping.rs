...
//!
//! Origin gives each stacked pane a scissor rect in device px and paints the axis/top layer
...
use origin_render::draw_list::{IRect, Prim};
use origin_render_gpui::{
    DeviceRect, GpuiChartRenderer, OriginViewport, PreparedOriginFrame, SceneOp, ScenePlan,
};
...
fn clip_rects_convert_to_logical_px_for_every_dpr_in_the_matrix() {
    ...
    for dpr in [1.0f32, 1.25, 1.5, 2.0, 2.5] {
        ...
        let viewport = OriginViewport::new(0.0, 0.0, 400.0, 300.0);

        for (device_v, logical) in [
            (
                clip.x,
                origin_render_gpui::to_logical_x(clip.x, viewport, dpr),
            ),
            (
                clip.y,
                origin_render_gpui::to_logical_y(clip.y, viewport, dpr),
            ),
        ] {
        ...
        let w_back = origin_render_gpui::to_logical_len(clip.w, dpr) * dpr;
        assert!(
...
#[test]
fn a_clip_offset_by_the_viewport_origin_lands_where_the_element_sits() {
    // AxiusFlow will place the chart element at an offset inside its window; the clip has to move
    // with it, not stay at the window origin.
    let viewport = OriginViewport::new(64.0, 40.0, 400.0, 300.0);
    let clip = DeviceRect::new(0.0, 0.0, 800.0, 360.0);
    assert_eq!(
        origin_render_gpui::to_logical_x(clip.x, viewport, 2.0),
        64.0,
        "the clip's left edge follows the element origin"
    );
    assert_eq!(
        origin_render_gpui::to_logical_y(clip.y, viewport, 2.0),
        40.0
    );
    assert_eq!(origin_render_gpui::to_logical_len(clip.w, 2.0), 400.0);
}
