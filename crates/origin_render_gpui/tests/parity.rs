...
use origin_render_gpui::{
    ExecutorOptions, GpuiChartRenderer, GpuiFrameMetrics, Paint, PreparedOriginFrame, SceneOp,
    ScenePlan,
};
...
fn text_runs_reach_both_backends_with_the_same_font_and_anchor() {
    ...
    assert_eq!(
        origin_render::draw_list::text_font_spec(run.size, &run.family, run.weight, run.italic),
        *font
    );
}
