...
use origin_render_gpui::{GpuiChartRenderer, PreparedOriginFrame, TextKey, TextMetrics};
...
fn footprint(r: &GpuiChartRenderer) -> usize {
    r.scratch_bytes()
        + r.plan().ops.capacity() * std::mem::size_of::<origin_render_gpui::SceneOp>()
        + r.plan().vertices.capacity() * std::mem::size_of::<origin_render_gpui::MeshVertex>()
}
...
fn a_live_append_replay_reaches_a_stable_footprint() {
    ...
    for _ in 0..32 {
        ...
        renderer
            .plan_frame(&PreparedOriginFrame::new(&frame), DPR as f32)
            .expect("frame plans");
    }
...
fn the_text_cache_plateaus_at_its_capacity_under_scrolling_labels() {
    use origin_render_gpui::TextCache;
    ...
    for i in 0..5_000 {
        let run = origin_render_gpui::TextRun {
            x: i as f32 * 0.37,
            y: 10.0,
            text: "42.50".into(),
