//! Official-GPUI integration demo and finite probe for Origin's complete engine frame.
//!
//! With `ORIGIN_PROBE_FRAMES` unset this is a real native application shell: grouped controls,
...
//! object bridge. With `ORIGIN_PROBE_FRAMES` set the original single-chart metrics/probe path is
...
//!
//! ```text
//! cargo run -p origin_render_gpui --features gpui-backend --example gpui_probe
//! ```
//!
//! Environment knobs:
//! - `ORIGIN_PROBE_BARS` — synthetic bars to load (default 500).
//! - `ORIGIN_PROBE_FRAMES` — quit after N painted frames and print a metrics summary. Unset runs
...
use origin_render::draw_list::{IRect, LineStyle, Prim, TextAlign};
...
impl Probe {
    ...
    fn inject_native_equivalents(&mut self) {
        ...
        if self.fixtures.plugin_watermark {
            top.push(Prim::Text {
                x: (sx + sw / 2) as f32,
                y: (sy + sh / 2) as f32,
                text: "ORIGIN · native plugin equivalent".into(),
        ...
        F: Fn(&str) -> f64,
    {
        ...
        println!(
            "origin probe viewport: canvas={width:.1}x{height:.1} logical, pane=({:.1},{:.1}) {:.1}x{:.1}, axes={:.1}/{:.1}, scissor={:?} device, dpr={scale_factor:.3}",
            self.engine.pane_left,
            0.0,
            self.engine.pane_w,
            self.engine.pane_h,
    ...
    fn begin_mouse_pan(&mut self, pane_x: f64) {
        ...
        // let the next `scroll_to` reuse that coast's original start point.
        self.engine.time_scale.end_scroll();
    ...
    fn on_mouse_down(
        ...
        self.update_cursor(chart_x, y);
        self.press_origin = Some((pane_x, y));
        self.press_moved = false;
    ...
    fn report(&self) {
        ...
        println!("--- origin_render_gpui probe ---");
...
impl Render for Probe {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .id("origin-chart-root")
            .track_focus(&focus)
            .key_context("OriginGpuiChart")
                ...
                canvas(
                    move |bounds: Bounds<gpui::Pixels>, window, cx| {
                        ...
                        prepaint_entity.update(cx, |probe: &mut Probe, _| {
                            ...
                            probe.viewport_origin = (origin_x, origin_y);
...
impl InteractiveDemo {
    fn new(bars: usize, cx: &mut Context<Self>) -> Self {
        let chart = cx.new(|cx| {
            ...
            probe.engine.series[0].title = "ORIGIN".into();
    ...
    fn apply_action(&mut self, action: DemoAction, cx: &mut Context<Self>) {
        match action {
            ...
            DemoAction::DrawingText => self.update_root(cx, |p| {
                p.update_drawing_template(|t| t.text = if t.text == "Native" { "Origin native" } else { "Native" }.into());
            }),
            ...
            DemoAction::TitleText => self.update_active(cx, |p| { p.engine.series[0].title = if p.engine.series[0].title == "ORIGIN" { "ASSET".into() } else { "ORIGIN".into() }; }),
            ...
            DemoAction::Watermark => self.update_root(cx, |p| {
                ...
                let text = if watermark.text.is_empty() { "ORIGIN" } else { &watermark.text };
            ...
            DemoAction::WatermarkText => self.update_root(cx, |p| { let current = p.engine.options.get().watermark.text; let text = if current == "ORIGIN" { "@origin/charts" } else { "ORIGIN" }; p.engine.options.apply_str(&format!(r#"{{"watermark":{{"text":"{text}"}}}}"#)).unwrap(); }),
...
fn main() {
    let budget = std::env::var("ORIGIN_PROBE_FRAMES")
        .ok()
        .and_then(|v| v.parse::<u64>().ok());
    let bars = std::env::var("ORIGIN_PROBE_BARS")
        .ok()
