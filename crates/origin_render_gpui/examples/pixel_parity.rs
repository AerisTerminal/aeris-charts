...
//!
//! ```text
//! cargo run --release -p origin_render_gpui --features gpui-backend --example pixel_parity
//! ```
//!
...
//! - `ORIGIN_PARITY_OUT`  — output directory (default `target/pixel_parity`).
//! - `ORIGIN_PARITY_TOL`  — per-channel tolerance for the diff (default 0 — exact).
...
const WINDOW_TITLE: &str = "origin-charts-pixel-parity-harness";
...
impl Harness {
    ...
    fn spawn_capture(&mut self) -> Receiver<String> {
        ...
        // Reference: Origin's existing native rasterizer over the identical prim list.
        let canvas =
            origin_native::render_prims(f.width, f.height, f.background, &f.prims, &f.points);
    ...
    fn finish_capture(&mut self, note: String) {
        ...
        // Diff with Origin's own comparator, at the requested tolerance.
        let (gpui_map, native_map) = (
            origin_native::load_png(gpui_png.to_str().unwrap_or_default()),
            origin_native::load_png(native_png.to_str().unwrap_or_default()),
        );
        let row = match (gpui_map, native_map) {
            (Ok(a), Ok(b)) => {
                if a.width() != b.width() || a.height() != b.height() {
                    ...
                    match origin_native::diff_pixmaps(&a, &b, self.tolerance) {
                        Some(d) => Row {
                            name: f.name.into(),
                            attribution: f.attribution.into(),
                            width: a.width(),
    ...
    fn report(&self) {
        println!("\n=== origin_render_gpui pixel parity ===");
        println!("reference: origin_native (tiny-skia) over the identical Prim list");
...
fn rgba_sha256_file(path: &std::path::Path) -> String {
    let Ok(pixmap) = origin_native::load_png(path.to_str().unwrap_or_default()) else {
        return String::new();
    };
...
fn main() {
    let out_dir = PathBuf::from(
        std::env::var("ORIGIN_PARITY_OUT").unwrap_or_else(|_| "target/pixel_parity".into()),
    );
    std::fs::create_dir_all(&out_dir).expect("output directory is creatable");
    let tolerance: u8 = std::env::var("ORIGIN_PARITY_TOL")
        .ok()
