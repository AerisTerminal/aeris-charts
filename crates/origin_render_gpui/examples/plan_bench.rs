...
//!
//! ```text
//! cargo run --release -p origin_render_gpui --example plan_bench
//! ```
//!
...
//!
//! Environment knobs:
//! - `ORIGIN_BENCH_ITERS` — timed iterations per fixture (default 200).
//! - `ORIGIN_BENCH_POINTS` — comma-separated source-point counts (default `10000,100000,1000000`).

use std::time::Instant;

use origin_engine::{ChartEngine, SeriesKind};
use origin_render_gpui::{GpuiChartRenderer, PreparedOriginFrame};
...
fn build_fixture(source_points: usize) -> Fixture {
    ...
    // `ORIGIN_BENCH_QUADS_ONLY=1` drops the line series, which isolates how much of the
    // scene-construction time is polyline tessellation versus quad emission.
    if std::env::var("ORIGIN_BENCH_QUADS_ONLY").as_deref() != Ok("1") {
...
fn main() {
    let iters: usize = std::env::var("ORIGIN_BENCH_ITERS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(200);
    let counts: Vec<usize> = std::env::var("ORIGIN_BENCH_POINTS")
        .ok()
        .map(|v| v.split(',').filter_map(|p| p.trim().parse().ok()).collect())
        .unwrap_or_else(|| vec![10_000, 100_000, 1_000_000]);

    println!(
        "origin_render_gpui scene-construction bench — {CSS_W}x{CSS_H} @ DPR {DPR}, {iters} iters/fixture"
    );
