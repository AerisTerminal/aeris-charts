...
//!
//! Environment:
//! - `ORIGIN_GPUI_CAPTURE_OUT` — required output PNG path.
//! - `ORIGIN_GPUI_CAPTURE_METADATA` — optional metadata JSON path (defaults beside the PNG).
...
const WINDOW_TITLE_PREFIX: &str = "origin-charts-gpui-webgpu-pane-capture";
...
impl Capture {
    fn new(scale_factor: f32, output: PathBuf, metadata: PathBuf, window_title: String) -> Self {
        let fixture = origin_native::engine_scene::parity_fixture();
        assert!(
        ...
        let theme = std::env::var("ORIGIN_GPUI_THEME").unwrap_or_else(|_| "light".into());
        assert!(
            matches!(theme.as_str(), "light" | "dark"),
            "ORIGIN_GPUI_THEME must be light or dark"
        );
        let spacing = std::env::var("ORIGIN_GPUI_BAR_SPACING").ok().map(|value| {
            value
                .parse::<f64>()
                .expect("ORIGIN_GPUI_BAR_SPACING must be numeric")
        });
        let feature = std::env::var("ORIGIN_GPUI_FEATURE").unwrap_or_else(|_| "base".into());
        assert!(
            matches!(feature.as_str(), "base" | "markers"),
            "ORIGIN_GPUI_FEATURE must be base or markers"
        );
        ...
        if feature == "markers" {
            use origin_engine::{marker_pos, marker_shape, Marker};
        ...
        let background_color =
            origin_render::color::Color::parse_css(&engine.options.get().layout.background.color)
                .expect("fixture background color is valid");
        let background = origin_render_gpui::backend::to_hsla(background_color);
    ...
    fn finish(&self, capture_note: &str) -> Result<(), String> {
        let image = origin_native::load_png(self.output.to_str().unwrap_or_default())
                ...
                move |bounds: Bounds<gpui::Pixels>, _prepared, window, cx| {
                    entity.update(cx, |capture: &mut Capture, cx| {
                        ...
                        capture
                            .renderer
                            .paint_frame(
                                &PreparedOriginFrame::new(&capture.frame),
                                viewport,
                                window.scale_factor(),
                                window,
                                cx,
...
fn main() {
    let output = PathBuf::from(
        std::env::var_os("ORIGIN_GPUI_CAPTURE_OUT")
            .expect("ORIGIN_GPUI_CAPTURE_OUT must name the output PNG"),
    );
    let metadata = std::env::var_os("ORIGIN_GPUI_CAPTURE_METADATA")
        .map(PathBuf::from)
