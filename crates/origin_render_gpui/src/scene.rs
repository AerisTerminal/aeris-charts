...
//!
//! Coordinates are **device (bitmap) px**, matching the `Prim` IR (`origin_render::draw_list`),
...
//! scale factor at the boundary — see [`crate::OriginViewport`].
...
/// the one case where Origin's ramp is *wider* than the geometry (`build_area_fill` floors its
...
pub struct TextRun {
    ...
    pub family: String,
    pub align: origin_render::draw_list::TextAlign,
