//! Optional GPUI executor for Origin's backend-neutral `DrawList<Prim>` frame.
//!
//! Origin's engine emits one ordered, backend-neutral primitive stream; WebGPU, Canvas2D and
...
//! an Origin chart without Origin growing a dependency on GPUI.
//!
//! # Layering
//!
//! ```text
...
//! ```
//!
//! Nothing below this crate knows GPUI exists. `origin_core`, `origin_engine` and `origin_render`
...
//! immutable result: [`PreparedOriginFrame`] borrows, so the adapter cannot mutate market data,
...
///
/// Mirrors exactly what the shipping WebGPU host composes (`origin_wasm::chart::inner_render`): the
...
pub struct PreparedOriginFrame<'a> {
...
/// themselves, whereas Origin's `Prim` IR is in device px with the DPR already baked in. The
/// adapter therefore divides device coordinates by the scale factor and offsets them by
/// `origin_x`/`origin_y` at the GPUI boundary — see [`GpuiChartRenderer::paint_frame`].
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct OriginViewport {
...
impl GpuiChartRenderer {
    ...
    pub fn plan_frame(
        ...
        // composition order the WebGPU host uses (`origin_wasm::chart::inner_render`).
...
pub fn to_logical_x(device_x: f32, viewport: OriginViewport, scale_factor: f32) -> f32 {
    viewport.origin_x + device_x / scale_factor
}

/// Convert a device-px y coordinate to GPUI logical px. See [`to_logical_x`].
pub fn to_logical_y(device_y: f32, viewport: OriginViewport, scale_factor: f32) -> f32 {
    viewport.origin_y + device_y / scale_factor
}

/// Convert a device-px length to GPUI logical px (no origin offset).
...
mod tests {
    use super::*;
    use origin_engine::FramePane;
    use origin_render::color::Color;
    use origin_render::draw_list::IRect;
    ...
    fn device_to_logical_round_trips_within_a_hundredth_of_a_pixel() {
        ...
        let viewport = OriginViewport::new(0.0, 0.0, 800.0, 600.0);
        for sf in [1.0f32, 1.25, 1.5, 2.0, 2.5] {
            for device in [0.0f32, 1.0, 37.0, 100.0, 999.0, 1919.0] {
                ...
                let back = (logical - viewport.origin_x) * sf;
                assert!(
    ...
    #[test]
    fn viewport_origin_offsets_the_chart_within_the_window() {
        let viewport = OriginViewport::new(40.0, 12.0, 800.0, 600.0);
