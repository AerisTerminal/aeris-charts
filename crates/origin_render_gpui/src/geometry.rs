...
//!
//! Every helper here reuses Origin's own pixel math rather than recomputing it:
...
//! - the anti-aliased subset calls `origin_render::line`'s tessellators
...
/// the DPR (see `origin_render_wgpu::tri_executor`), so re-scaling here would double-apply it.
...
///
/// Origin's tessellators write into caller-provided `Vec`s. Allocating those fresh per prim made
    ...
    pub fn capacity_bytes(&self) -> usize {
        self.points.capacity() * std::mem::size_of::<LinePoint>()
            + self.stroke.vertices.capacity()
                * std::mem::size_of::<origin_render::line::LineVertex>()
            + self.area.vertices.capacity() * std::mem::size_of::<origin_render::line::LineVertex>()
...
pub(crate) fn dashed_polyline_meshes(
    ...
    let expanded = origin_render::line::expand_line(&scratch.points, line_type);
    let pattern: Vec<f64> = pattern.iter().map(|&len| len as f64).collect();
    for run in origin_render::line::dash_split(&expanded, &pattern) {
...
///
/// Origin shades each vertex with `t = clamp((y - top) / span, 0, 1)` where
...
/// Origin's ramp is *wider* than the geometry, so the bottom stop is never reached; we compress it
/// by substituting the color Origin would actually have produced at the mesh's bottom edge. Both
...
fn area_gradient(
    ...
    // How far along Origin's ramp the mesh's bottom edge actually sits.
...
mod tests {
    ...
    #[test]
    fn area_gradient_compresses_when_origin_floors_its_span() {
        // A fill only 0.5 px tall: Origin's `span` floors at 1.0, so its bottom stop is only half
