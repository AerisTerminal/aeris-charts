...
//!
//! This is the GPUI counterpart of `origin_render::canvas2d::execute` and
//! `origin_render_wgpu`'s quad/tri executors, and it deliberately mirrors their control flow arm
...
use origin_render::draw_list::{LineStyle, Prim};
...
pub struct ExecutorOptions {
    /// Express `Prim::RoundRect` as a single GPUI rounded `paint_quad` instead of Origin's
    /// tessellated polygon.
    ///
    /// `false` (the default) reproduces `origin_render_wgpu`'s `round_rect_polygon` triangle-for-
    ...
    match prim {
        ...
        Prim::HLine {
            ...
            dash_spans(*style, w, *x0, *x1, |a, b| {
                if let Some(rect) = irect(origin_render::draw_list::IRect {
                    x: a,
                    y: top,
                    w: b - a,
                    h: w,
        ...
        Prim::Polyline {
            ...
            // first, exactly as `origin_engine::frame::series_geometry::push_line_stroke` does for
        ...
        Prim::RoundRect {
            ...
            // Origin-generated strips: the wgpu executor's exact construction — the border as the
...
fn push_quad(plan: &mut ScenePlan, metrics: &mut GpuiFrameMetrics, rect: DeviceRect, fill: Paint) {
    ...
    plan.ops.push(SceneOp::Quad {
        ...
        corner_radii: [0.0; 4],
        border_width: 0.0,
        border_color: origin_render::color::Color::rgba(0, 0, 0, 0),
    });
    metrics.quads += 1;
    metrics.ops += 1;
}
...
mod tests {
    ...
    use origin_render::draw_list::{Gradient, IRect, LineType, TextAlign};
    ...
    #[test]
    fn round_rect_defaults_to_origin_tessellation_not_a_native_quad() {
