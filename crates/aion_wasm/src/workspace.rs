//! Multi-chart workspace host: the wasm surface over `aion_engine::Workspace` (the split-grid
...
use aion_engine::{SplitDirection, Workspace, WorkspaceError};
use wasm_bindgen::prelude::*;

/// Host-facing workspace handle. Clock is host-injected seconds (Date.now()/1000).
#[wasm_bindgen]
pub struct AionWorkspace {
    workspace: Workspace,
}
