...
//!
//! Recorded from `nucleuscharts_render::draw_list`, `nucleuscharts_render::canvas2d`, `nucleuscharts_wasm::text_cache`
//! and `nucleuscharts_native`, which are the shipping text paths:
//!
//! | Input | Current behavior |
//! |---|---|
//! | Font shorthand | `nucleuscharts_render::draw_list::text_font_spec` → `"[italic ]{weight} {size}px {family}"` |
...
//! | Baseline derivation | `nucleuscharts_native`: `baseline = y + (ascent + descent) / 2` (ab_glyph signs) |
//! | Label bounds / clipping | The owning layer's clip rect; text is never clipped per-glyph |
//! | Numeric formatting, locale, timezone | Resolved upstream in `nucleuscharts_core::format`; the IR carries final strings |
...
//! coordinates: it is the same discipline `nucleuscharts_wasm::text_cache::TextRunKey` already uses, and
...
//! eviction policy needed by a Nucleus-owned raster path (Stage 3).

use std::collections::HashMap;

use nucleuscharts_render::color::Color;
use nucleuscharts_render::draw_list::{text_font_spec, TextAlign};
...
/// What a shaper measured for a run. Sign convention matches `ab_glyph` and Nucleus's native
...
/// at it — the same math `nucleuscharts_native::fill_text` uses.
...
///
/// `center_y + (ascent + descent) / 2` — identical to `nucleuscharts_native`'s formula, so the GPUI and
