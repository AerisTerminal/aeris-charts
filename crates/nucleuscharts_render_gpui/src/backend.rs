...
//! before inserting into the scene. Nucleus's IR is already in device px, so every coordinate is
...
use crate::{GpuiChartRenderer, GpuiRenderError, NucleusViewport, PreparedNucleusFrame};
...
/// Nucleus's 8-bit sRGB color as a GPUI `Hsla`.
///
...
impl Transform {
    fn new(viewport: NucleusViewport, scale_factor: f32) -> Self {
        Self {
...
impl GpuiChartRenderer {
    ...
    /// [`PreparedNucleusFrame`]): this method only reads it, so no model, layout, or indicator
...
impl GpuiChartRenderer {
    ...
    /// must put the *identical* prim list through GPUI and through `nucleuscharts_native` to attribute any
    /// residual. Hosts should use [`GpuiChartRenderer::paint_frame`].
    pub fn paint_prims(
        &mut self,
        prims: &[nucleuscharts_render::draw_list::Prim],
        points: &[[f32; 2]],
        viewport: NucleusViewport,
        scale_factor: f32,
        window: &mut Window,
...
mod tests {
    ...
    fn transform_maps_device_px_into_logical_px() {
        let t = Transform::new(NucleusViewport::new(10.0, 20.0, 800.0, 600.0), 2.0);
