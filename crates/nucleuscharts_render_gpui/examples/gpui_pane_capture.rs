...
//!
//! Environment:
//! - `NUCLEUSCHARTS_GPUI_CAPTURE_OUT` — required output PNG path.
//! - `NUCLEUSCHARTS_GPUI_CAPTURE_METADATA` — optional metadata JSON path (defaults beside the PNG).
...
use nucleuscharts_engine::ChartFrame;
use nucleuscharts_render_gpui::{GpuiChartRenderer, NucleusViewport, PreparedNucleusFrame};

const WINDOW_TITLE_PREFIX: &str = "nucleuscharts-gpui-webgpu-pane-capture";
...
impl Capture {
    fn new(scale_factor: f32, output: PathBuf, metadata: PathBuf, window_title: String) -> Self {
        let fixture = nucleuscharts_native::engine_scene::parity_fixture();
        assert!(
        ...
        let theme = std::env::var("NUCLEUSCHARTS_GPUI_THEME").unwrap_or_else(|_| "light".into());
        assert!(
            matches!(theme.as_str(), "light" | "dark"),
            "NUCLEUSCHARTS_GPUI_THEME must be light or dark"
        );
        let spacing = std::env::var("NUCLEUSCHARTS_GPUI_BAR_SPACING")
            .ok()
            .map(|value| {
                value
                    .parse::<f64>()
                    .expect("NUCLEUSCHARTS_GPUI_BAR_SPACING must be numeric")
            });
        ...
        if theme == "dark" {
            let surface = nucleuscharts_core::style::DARK_SURFACE_CSS;
            let text = nucleuscharts_core::style::DARK_AXIS_TEXT_CSS;
            let border = nucleuscharts_core::style::DARK_BORDER_CSS;
            let options = format!(
        ...
        if feature == "markers" {
            use nucleuscharts_engine::{marker_pos, marker_shape, Marker};
        ...
        let background = nucleuscharts_render_gpui::backend::to_hsla(background_color);
    ...
    fn finish(&self, capture_note: &str) -> Result<(), String> {
        let image = nucleuscharts_native::load_png(self.output.to_str().unwrap_or_default())
                ...
                move |bounds: Bounds<gpui::Pixels>, _prepared, window, cx| {
                    entity.update(cx, |capture: &mut Capture, cx| {
                        ...
                        capture
                            .renderer
                            .paint_frame(
                                &PreparedNucleusFrame::new(&capture.frame),
                                viewport,
                                window.scale_factor(),
                                window,
                                cx,
...
fn main() {
    let output = PathBuf::from(
        std::env::var_os("NUCLEUSCHARTS_GPUI_CAPTURE_OUT")
            .expect("NUCLEUSCHARTS_GPUI_CAPTURE_OUT must name the output PNG"),
    );
    let metadata = std::env::var_os("NUCLEUSCHARTS_GPUI_CAPTURE_METADATA")
        .map(PathBuf::from)
