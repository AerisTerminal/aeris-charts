...
//!
//! ```text
//! cargo run --release -p aeris_charts_render_gpui --features gpui-backend --example pixel_parity
//! ```
//!
...
//! - `AERIS_CHARTS_PARITY_TOL`  — per-channel tolerance for the diff (default 0 — exact).
...
impl Harness {
    ...
    fn finish_capture(&mut self, note: String) {
        ...
        // Diff with Aeris's own comparator, at the requested tolerance.
        let (gpui_map, native_map) = (
            aeris_charts_native::load_png(gpui_png.to_str().unwrap_or_default()),
            aeris_charts_native::load_png(native_png.to_str().unwrap_or_default()),
        );
        let row = match (gpui_map, native_map) {
            (Ok(a), Ok(b)) => {
                if a.width() != b.width() || a.height() != b.height() {
                    ...
                    match aeris_charts_native::diff_pixmaps(&a, &b, self.tolerance) {
                        Some(d) => Row {
                            name: f.name.into(),
                            attribution: f.attribution.into(),
                            width: a.width(),
...
fn rgba_sha256_file(path: &std::path::Path) -> String {
    let Ok(pixmap) = aeris_charts_native::load_png(path.to_str().unwrap_or_default()) else {
        return String::new();
    };
