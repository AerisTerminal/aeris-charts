...
//!
//! Aeris gives each stacked pane a scissor rect in device px and paints the axis/top layer
...
use aeris_charts_render::draw_list::{IRect, Prim};
use aeris_charts_render_gpui::{
    AerisViewport, DeviceRect, GpuiChartRenderer, PreparedAerisFrame, SceneOp, ScenePlan,
};
...
fn clip_rects_convert_to_logical_px_for_every_dpr_in_the_matrix() {
    ...
    for dpr in [1.0f32, 1.25, 1.5, 2.0, 2.5] {
        ...
        let viewport = AerisViewport::new(0.0, 0.0, 400.0, 300.0);

        for (device_v, logical) in [
            (
                clip.x,
                aeris_charts_render_gpui::to_logical_x(clip.x, viewport, dpr),
            ),
            (
                clip.y,
                aeris_charts_render_gpui::to_logical_y(clip.y, viewport, dpr),
            ),
        ] {
        ...
        let w_back = aeris_charts_render_gpui::to_logical_len(clip.w, dpr) * dpr;
        assert!(
...
fn a_clip_offset_by_the_viewport_offset_lands_where_the_element_sits() {
    // Aeris Terminal will place the chart element at an offset inside its window; the clip has to move
    // with it, not stay at the window offset.
    let viewport = AerisViewport::new(64.0, 40.0, 400.0, 300.0);
    let clip = DeviceRect::new(0.0, 0.0, 800.0, 360.0);
    assert_eq!(
        aeris_charts_render_gpui::to_logical_x(clip.x, viewport, 2.0),
        64.0,
        "the clip's left edge follows the element offset"
    );
    assert_eq!(
        aeris_charts_render_gpui::to_logical_y(clip.y, viewport, 2.0),
        40.0
    );
    assert_eq!(aeris_charts_render_gpui::to_logical_len(clip.w, 2.0), 400.0);
}
