        ...
        for index in 0..self.indicators.len() {
            ...
            match binding.kind {
                ...
                IndicatorKind::Rsi { period } => {
                    let values = aion_indicators::rsi(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Macd { fast, slow, signal } => {
                    let points = aion_indicators::macd(&close, fast, slow, signal);
                ...
                IndicatorKind::Stochastic { k_period, d_period } => {
                    let points =
                        aion_indicators::stochastic(&high, &low, &close, k_period, d_period);
                ...
                IndicatorKind::Atr { period } => {
                    let values = aion_indicators::atr(&high, &low, &close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Vwap => {
                    let volumes = binding
                        ...
                        .map(|(_, v)| v[3].to_vec())
                        .unwrap_or_default();
                    let values = aion_indicators::vwap(&times, &high, &low, &close, &volumes);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Wma { period } => {
                    let values = aion_indicators::wma(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
            }
