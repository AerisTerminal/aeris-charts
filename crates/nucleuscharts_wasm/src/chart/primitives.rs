...
//! top — pane offsets included, matching the pane's scissor rect). This makes the converters
//! agree with engine geometry exactly: the engine rounds `(coord * ratio)` per axis and adds
//! the integer pane offset, and `round(a + k) == round(a) + k` for integer `k`.
//!
...
//! `CanvasRenderingTarget2D`; Nucleus renderers record backend-neutral commands instead (the
//! design's locked A-first hybrid). reference pane primitives carry no axis views (those exist only
//! on `ISeriesPrimitiveBase`); Nucleus accepts `price_axis_views`/`time_axis_views` on pane
...
use nucleuscharts_core::scale::price_scale_core::PriceScaleCore;
use nucleuscharts_core::style::{DEFAULT_AXIS_TEXT_RGB, DEFAULT_CROSSHAIR_RGB};
...
pub(super) struct PrimitiveScaleSnapshot {
    pub(super) time_scale: nucleuscharts_core::scale::time_scale_core::TimeScaleCore,
    ...
    /// (nucleuscharts_engine frame/mod.rs `build_frame_into`) so converter output matches engine geometry.
    pub(super) hpr: f64,
    vpr: f64,
}
...
impl ChartInner {
                ...
                if let Ok(hook) = hook.dyn_into::<js_sys::Function>() {
                    if let Err(error) = hook.call0(&obj) {
                        web_sys::console::warn_1(
                            &format!("nucleuscharts: pane primitive `update_all_views` threw — {error:?}")
                                .into(),
                        );
                    }
                }
    ...
    pub(super) fn hover_at(&mut self, x_css: f64, y_css: f64) -> String {
        ...
        let mut best_series: Option<nucleuscharts_engine::SeriesHit> = None;
        for &id in order.iter().rev() {
            if let Some(hit) = &best_primitive {
        ...
        for index in 0..self.series_primitives.len() {
            ...
            let result = match hook.call2(
                ...
                Err(error) => {
                    web_sys::console::warn_1(
                        &format!(
                            "nucleuscharts: series primitive `autoscale_info` threw — {error:?}"
                        )
                        .into(),
                    );
                    continue;
            ...
            if let Ok(hook) = js_sys::Reflect::get(&obj, &"update_all_views".into()) {
                if let Ok(hook) = hook.dyn_into::<js_sys::Function>() {
                    if let Err(error) = hook.call0(&obj) {
                        web_sys::console::warn_1(
                            &format!(
                                "nucleuscharts: series primitive `update_all_views` threw — {error:?}"
                            )
                            .into(),
                        );
                    }
    ...
    fn run_primitive_view(
        ...
        if let Err(error) = renderer.call1(view, &ctx) {
            web_sys::console::warn_1(
                &format!("nucleuscharts: pane primitive renderer threw — {error:?}").into(),
            );
            return;
        }
        ...
        if !decoded.warnings.is_empty() {
            web_sys::console::warn_1(
                &format!(
                    "nucleuscharts: pane primitive skipped {} command(s) — {}",
...
fn call_view_array_with(
    ...
    .map_err(|error| {
        web_sys::console::warn_1(
            &format!("nucleuscharts: pane primitive `{hook}` threw — {error:?}").into(),
        );
    })
    .ok()?;
...
/// One primitive's non-null `hit_test` result (reference `PrimitiveHoveredItem`, reduced to Nucleus's
...
fn call_hit_test(obj: &js_sys::Object, x: f64, y: f64) -> Option<PrimitiveHit> {
    let value = js_sys::Reflect::get(obj, &"hit_test".into())
        ...
        .map_err(|error| {
            web_sys::console::warn_1(
                &format!("nucleuscharts: primitive `hit_test` threw — {error:?}").into(),
            );
        })
        .ok()?;
