...
impl BoundRing {
    ...
    pub(super) fn drain(&mut self, engine: &mut nucleuscharts_engine::ChartEngine) -> DrainOutcome {
    ...
    REPORTED.with(|reported| {
        if !reported.replace(true) {
            web_sys::console::warn_1(
                &format!(
                    "nucleuscharts: ring source cursor read failed, skipping drains ({error:?})"
                )
                .into(),
            );
        }
