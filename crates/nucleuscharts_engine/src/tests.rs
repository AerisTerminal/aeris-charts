...
fn crosshair_geometry_is_host_independent() {
    ...
    assert!(frame.panes[0]
        .main
        .iter()
        .any(|p| matches!(p, nucleuscharts_render::draw_list::Prim::VLine { .. })));
    assert!(frame.panes[0]
        .main
        .iter()
        .any(|p| matches!(p, nucleuscharts_render::draw_list::Prim::HLine { .. })));
    assert!(frame.panes[0]
        .main
        .iter()
        .any(|p| matches!(p, nucleuscharts_render::draw_list::Prim::Circle { .. })));
...
fn frame_pane_top_layer_is_retained_per_frame() {
    ...
    frame.panes[0]
        .top_prims
        .push(nucleuscharts_render::draw_list::Prim::Rect {
            rect: nucleuscharts_render::draw_list::IRect {
                x: 0,
                y: 0,
                w: 4,
                h: 4,
            },
            color: nucleuscharts_render::color::Color::rgb(0xff, 0x00, 0x00),
        });
...
fn price_format_labels_follow_their_owning_series() {
    ...
    let line_id = chart.create_price_line(
        ...
        Color::rgb(0, 0, 0),
        1,
        nucleuscharts_render::draw_list::LineStyle::Solid,
        "",
    );
    assert!(line_id > 0);
    let axis = chart.build_axis_frame(
...
#[test]
fn theme_switch_uses_nucleus_tokens_without_replacing_market_data() {
    let mut chart = ChartEngine::new(800.0, 500.0, 1.0);
