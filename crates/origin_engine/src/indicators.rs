...
//! ordinary engine series (`origin_indicators` holds the pure math). Extracted from `lib.rs`.
        ...
        for index in 0..self.indicators.len() {
            ...
            match binding.kind {
                IndicatorKind::Sma { period } => {
                    let values = origin_indicators::sma(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Ema { period } => {
                    let values = origin_indicators::ema(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Bollinger { period, deviation } => {
                    let values = origin_indicators::bollinger(&close, period, deviation);
                ...
                IndicatorKind::Rsi { period } => {
                    let values = origin_indicators::rsi(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Macd { fast, slow, signal } => {
                    let points = origin_indicators::macd(&close, fast, slow, signal);
                ...
                IndicatorKind::Stochastic { k_period, d_period } => {
                    let points =
                        origin_indicators::stochastic(&high, &low, &close, k_period, d_period);
                ...
                IndicatorKind::Atr { period } => {
                    let values = origin_indicators::atr(&high, &low, &close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Vwap => {
                    let volumes = binding
                        ...
                        .map(|(_, v)| v[3].to_vec())
                        .unwrap_or_default();
                    let values = origin_indicators::vwap(&times, &high, &low, &close, &volumes);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
                IndicatorKind::Wma { period } => {
                    let values = origin_indicators::wma(&close, period);
                    self.install_indicator_output(binding.outputs[0], &times, &values);
                }
            }
