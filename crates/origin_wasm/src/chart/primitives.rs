...
//! `CanvasRenderingTarget2D`; Origin renderers record backend-neutral commands instead (the
//! design's locked A-first hybrid). reference pane primitives carry no axis views (those exist only
//! on `ISeriesPrimitiveBase`); Origin accepts `price_axis_views`/`time_axis_views` on pane
...
pub(super) struct PrimitiveScaleSnapshot {
    pub(super) time_scale: origin_core::scale::time_scale_core::TimeScaleCore,
    ...
    /// (origin_engine frame/mod.rs `build_frame_into`) so converter output matches engine geometry.
    pub(super) hpr: f64,
    vpr: f64,
}
...
impl ChartInner {
                ...
                if let Ok(hook) = hook.dyn_into::<js_sys::Function>() {
                    if let Err(error) = hook.call0(&obj) {
                        web_sys::console::warn_1(
                            &format!("origin: pane primitive `update_all_views` threw — {error:?}")
                                .into(),
                        );
                    }
                }
    ...
    pub(super) fn hover_at(&mut self, x_css: f64, y_css: f64) -> String {
        ...
        let mut best_series: Option<origin_engine::SeriesHit> = None;
        for &id in order.iter().rev() {
            if let Some(hit) = &best_primitive {
    ...
    pub(super) fn collect_series_primitive_autoscale(&mut self) {
        ...
        for index in 0..self.series_primitives.len() {
            ...
            let result = match hook.call2(
                ...
                Err(error) => {
                    web_sys::console::warn_1(
                        &format!("origin: series primitive `autoscale_info` threw — {error:?}")
                            .into(),
                    );
                    continue;
                }
    ...
    fn run_primitive_view(
        ...
        if let Err(error) = renderer.call1(view, &ctx) {
            web_sys::console::warn_1(
                &format!("origin: pane primitive renderer threw — {error:?}").into(),
            );
            return;
        }
        ...
        if !decoded.warnings.is_empty() {
            web_sys::console::warn_1(
                &format!(
                    "origin: pane primitive skipped {} command(s) — {}",
...
fn call_view_array_with(
    ...
    .map_err(|error| {
        web_sys::console::warn_1(
            &format!("origin: pane primitive `{hook}` threw — {error:?}").into(),
        );
    })
    .ok()?;
...
/// One primitive's non-null `hit_test` result (reference `PrimitiveHoveredItem`, reduced to Origin's
...
fn call_hit_test(obj: &js_sys::Object, x: f64, y: f64) -> Option<PrimitiveHit> {
    let value = js_sys::Reflect::get(obj, &"hit_test".into())
        ...
        .map_err(|error| {
            web_sys::console::warn_1(
                &format!("origin: primitive `hit_test` threw — {error:?}").into(),
            );
        })
        .ok()?;
