...
use origin_engine::{CustomSeriesFrameValues, CustomSeriesLastValue, SeriesEntry};
        ...
        return match check.call1(view, item) {
            ...
            Err(error) => {
                web_sys::console::warn_1(
                    &format!("origin: custom series `is_whitespace` threw — {error:?}").into(),
                );
                false
            }
        };
...
fn custom_price_values(
    ...
    let result = match builder.call1(view, item) {
        ...
        Err(error) => {
            web_sys::console::warn_1(
                &format!("origin: custom series `price_value_builder` threw — {error:?}").into(),
            );
            return (Vec::new(), None);
        }
    };
    let Ok(array) = result.dyn_into::<js_sys::Array>() else {
        web_sys::console::warn_1(
            &"origin: custom series `price_value_builder` must return an array of numbers".into(),
        );
        return (Vec::new(), None);
    };
    ...
    js_sys::Reflect::get(item, &"color".into())
        ...
        .unwrap_or_else(|| {
            series
                ...
                .unwrap_or(origin_engine::DEFAULT_LINE_COLOR)
        })
}
...
impl ChartInner {
    ...
    pub(super) fn add_custom_series(&mut self, view: js_sys::Object, adopt_primary: bool) -> u32 {
        ...
        {
            web_sys::console::warn_1(
                &"origin: add_custom_series rejected — the pane view needs `price_value_builder` and `render` functions".into(),
            );
            return u32::MAX;
        }
    ...
    pub(super) fn set_custom_series_data(&mut self, id: u32, items: js_sys::Array) {
        let Some(position) = self.custom_series.iter().position(|e| e.series == id) else {
            web_sys::console::warn_1(
                &format!("origin: set_custom_series_data for unknown custom series {id}").into(),
            );
            return;
        };
        ...
        if !report.is_clean() {
            web_sys::console::warn_1(
                &format!(
                    "origin: set_custom_series_data sanitized data — accepted {}, dropped {} invalid, {} duplicate{}",
                    report.accepted,
    ...
    pub(super) fn update_custom_series_item(&mut self, id: u32, item: JsValue) {
        let Some(position) = self.custom_series.iter().position(|e| e.series == id) else {
            web_sys::console::warn_1(
                &format!("origin: update_custom_series_item for unknown custom series {id}").into(),
            );
            return;
        };
        ...
        let Some(time) = time else {
            web_sys::console::warn_1(
                &"origin: update_custom_series_item dropped a non-finite time".into(),
            );
            return;
        };
        let entry = &mut self.custom_series[position];
        ...
        for entry in entries {
            ...
            if let Some(destroy) = custom_hook(&entry.view, "destroy") {
                if let Err(error) = destroy.call0(&entry.view) {
                    web_sys::console::warn_1(
                        &format!("origin: custom series `destroy` hook threw — {error:?}").into(),
                    );
                }
            }
        }
    ...
    pub(super) fn run_custom_series(&mut self) {
        ...
        for p in pending {
            ...
            if let Err(error) = renderer.call1(&view, &ctx) {
                web_sys::console::warn_1(
                    &format!("origin: custom series `render` threw — {error:?}").into(),
                );
                continue;
            }
            ...
            if !decoded.warnings.is_empty() {
                web_sys::console::warn_1(
                    &format!(
                        "origin: custom series skipped {} command(s) — {}",
