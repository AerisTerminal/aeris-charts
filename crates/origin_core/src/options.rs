...
impl Default for GridLineOptions {
    fn default() -> Self {
        Self {
            ...
            // Deliberate divergence from the reference (visible): Origin charts ship grid-free.
            visible: false,
        }
    }
}
...
impl Default for CrosshairLineOptions {
    fn default() -> Self {
        Self {
            ...
            width: 1.0,
            // Deliberate divergence from the reference's LargeDashed default: Origin crosshair
...
/// shape). Origin draws it on the shared Canvas2D overlay above the series (a deliberate divergence
...
mod tests {
    ...
    #[test]
    fn defaults_match_origin_style() {
    ...
    fn watermark_patch_merges_camelcase_keys() {
        ...
        store.apply(&json!({
            "watermark": { "visible": true, "text": "ORIGIN", "fontSize": 64, "horzAlign": "left" },
        }));
        let o = store.get();
        assert!(o.watermark.visible);
        assert_eq!(o.watermark.text, "ORIGIN");
