...
fn indicator_source_scaling_rows() -> Vec<serde_json::Value> {
    ...
    for source_count in [1, 2, 4, 8, 16] {
        ...
        sources.extend(
            (1..source_count).map(|_| chart.add_series(aeris_charts_engine::SeriesKind::Line)),
        );
...
fn multi_series_memory_rows() -> Vec<serde_json::Value> {
    ...
    for aligned in [true, false] {
        for count in [1, 2, 4, 8, 16] {
            ...
            let mut ids = vec![0];
            ids.extend(
                (1..count).map(|_| chart.add_series(aeris_charts_engine::SeriesKind::Candlestick)),
            );
...
fn dense_upload_breakdown(full_history: bool) -> serde_json::Value {
    ...
    let mut group = aeris_charts_render_wgpu::DrawGroup::default();
    aeris_charts_render_wgpu::prims_to_group(
    ...
    let triangle_bytes =
        group.tris.len() * std::mem::size_of::<aeris_charts_render_wgpu::TriVertex>();
    let quad_bytes =
        group.quads.len() * std::mem::size_of::<aeris_charts_render_wgpu::QuadInstance>();
    let textured_bytes =
        group.tex_quads.len() * std::mem::size_of::<aeris_charts_render_wgpu::TexQuadInstance>();
    ...
    for points in [10_000, 100_000, 500_000, 1_000_000] {
        ...
        for series_count in [1, 4, 8] {
            for (zoom, visible_points) in [
                ...
                ids.extend(
                    (1..series_count)
                        .map(|_| chart.add_series(aeris_charts_engine::SeriesKind::Candlestick)),
                );
                for &id in &ids {
                    install_series(&mut chart, id, &columns, 0.0);
                }
