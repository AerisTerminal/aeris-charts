...
use aeris_charts_engine::{ChartEngine, ChartFrame, SeriesKind};
...
fn main() {
    let bars: usize = std::env::var("AERIS_CHARTS_BARS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1_000_000);
    let iterations: usize = std::env::var("AERIS_CHARTS_INTERACTIONS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(240);
