 ...
 * split count, per-cell ages, and the paywall cap — is engine-owned (`nucleuscharts_engine::Workspace`,
  ...
  const reconcile = () => {
    ...
    const build = (node: layout_node): HTMLElement => {
      if (node.kind === "cell") {
        ...
        if (!record) throw new Error(`nucleuscharts grid: engine cell ${node.id} has no chart`);
        record.slot.style.display = "";
        return record.slot;
      }
