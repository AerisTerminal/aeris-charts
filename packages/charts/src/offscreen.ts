...
function split_offscreen_options(options: offscreen_chart_options): {
  ...
  for (const key of UNSUPPORTED_WORKER_OPTIONS) {
    if (key in raw) throw new Error(`origin: worker charts do not support option ${key}`);
  }
  const layout = raw.layout as { panes?: Record<string, unknown> } | undefined;
  if (layout?.panes && "enableResize" in layout.panes) {
    throw new Error("origin: worker charts do not support option layout.panes.enableResize");
  }
...
export class offscreen_chart {
  ...
  constructor(
    private readonly wasm: OriginChart,
    ...
    // wasm export still owns `OriginChart`; re-entering wasm from that synchronous event would trip
    ...
    globalThis.addEventListener("origin-chart-backend-lost", this.backend_loss_handler);
  }
    ...
    if (backend !== this.last_backend) {
      ...
      for (const handler of this.backend_change_handlers) {
        try {
          ...
          console.error("origin: offscreen backend-change handler threw", error);
        }
      }
    }
  }
  ...
  remove(): void {
    ...
    globalThis.removeEventListener("origin-chart-backend-lost", this.backend_loss_handler);
  ...
  private assert_live(): void {
    if (this.removed) throw new Error("origin: offscreen chart has been removed");
  }
}
